...
# ════════════════════════════════════════════════════════════════════════════════
# CHANGE 1 — Add Optional fields to CloneVideoRequest
# ════════════════════════════════════════════════════════════════════════════════

# Optional import check — add it to the BaseModel import if it doesn't exist
if "from typing import Optional" not in src and "Optional" not in src:
    ...
    # Add directly to header section
    src = "from typing import Optional\n" + src
    print("[patch] Optional import başa eklendi")

# Add Optional fields after the last required field in CloneVideoRequest.
# Goal: Add new block after user_id line.
OLD_CVR = '    user_id:   int = Field(default=0, description='
NEW_CVR_SUFFIX = '''

    # ── Predictive Intelligence - Optional Fields ──────────────────────
    # extra="ignore" is already defined — it won't get a 422 without sending any.
...
# Line-based search (encoding-resistant)
...
for i, line in enumerate(lines):
    ...
    if not found_cvr and 'user_id' in line and 'Field(default=0' in line and 'CloneVideoRequest' not in line:
        # Look until the next empty line or end of class
        # Simple approach: add Optional block after this line
        # Only add while in CloneVideoRequest
        # Look back: is there a CloneVideoRequest in the last 30 lines?
...
# ════════════════════════════════════════════════════════════════════════════════
# CHANGE 2 — Update _call_groq_clone() signature + 5-mode prompt
# ════════════════════════════════════════════════════════════════════════════════
...
# ── Toggle 5-Mode Prompt block ───────────────────── ─────────────────────
# Replace existing VIRAL_THRESHOLD block with 5-mode system
...
NEW_THRESHOLD_BLOCK = '''    thumbnail_rule = _build_thumbnail_rule(content_type)
    ...
    # ── Subscriber Penetration Context ────────────────────── ──────────────────────
    penetration_context = ""
    ...
    # ── 5-Level Tier Detection ─────────────────────── ────────────────────────
    # If tier is not received from Frontend, calculate based on views
    ...
    # AI tone and anatomy directive by Tier
...
# ── Adapted is_viral references in prompt to new tier ──────────
# views_info definitions are now above — remove the old if is_viral block in the prompt
...
# ── Replace old is_viral reference with new one in prompt f-string ─────
# Remove inline conditional as in "{'Why did it explode..."
...
# ── Improve prompt template: add extra context ────────────────────────────
...
if OLD_VIEWS_LINE in src:
    ...
    # Regex for Turkish character encoding difference
...
# ── Add tone directive at the beginning of the prompt ─────────────────────────────────────
...
# ════════════════════════════════════════════════════════════════════════════════
# CHANGE 3 — Update _call_groq_clone call site
# ════════════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════════════
# CHANGE 4 — Update _call_groq_debate signature
# ════════════════════════════════════════════════════════════════════════════════
...
# Find in _call_groq_debate context (there may be more than one, choose the correct one)
debate_func_pos = src.find("async def _call_groq_debate(")
if debate_func_pos != -1:
    # Update in function signature region
    sig_search_area = src[debate_func_pos: debate_func_pos + 600]
    if "    views: int = 0,\n) -> dict:" in sig_search_area:
        # Change the first occurrence in this section
...
# ════════════════════════════════════════════════════════════════════════════════
# CHANGE 5 — _call_groq_debate 5-mode anatomy directive
# ════════════════════════════════════════════════════════════════════════════════
...
NEW_DEBATE_VIRAL = """    # ── Suni Artış Uyarısı (Hakem için) ────────────────────────────────────
    ...
    # ── Penetration Context (For Referee) ─────────────────────────────────────
    debate_pen_ctx = ""
    if penetration_ratio is not None:
...
if OLD_DEBATE_VIRAL in src:
    ...
    # Simple is_viral block search (regex)
    ...
    if m:
        ...
        if debate_pos != -1 and m.start() > debate_pos:
            # Find end of block (end of else block)
...
# ── Add extra context to the Debate referee prompt ─────────────────────────────────
...
# Just change the views line in the debate function (whatever comes after the clone prompt)
...
# ════════════════════════════════════════════════════════════════════════════════
# CHANGE 6 — Update _call_groq_debate call site
# ════════════════════════════════════════════════════════════════════════════════
