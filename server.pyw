...
# --- VERİTABANI YOLU (DİNAMİK & KESİN) ---
if getattr(sys, 'frozen', False):
    # EXE modunda: EXE'nin yanındaki klasör
    db_path = os.path.join(os.path.dirname(sys.executable), 'channels.db')
else:
    # Geliştirme modunda: script'in yanındaki klasör
...
def send_verification_email(to_email: str, code: str, lang: str = "tr") -> bool:
    ...
    try:
        # SMTP bilgisi artık async endpoint'ten alınıp parametre olarak geçilmeli
...
— Equipo YouTube Analytics Pro"""
        ...
        with smtplib.SMTP_SSL('smtp.gmail.com', 465) as server:
            server.login(smtp_email, smtp_password)
            server.sendmail(smtp_email, to_email, msg.as_string())
        app_logger.info(f"📧 Rapor e-postası gönderildi: {to_email}")
        return True
    except Exception as e:
        app_logger.error(f"Rapor e-postası gönderilemedi: {type(e).__name__}: {e}")
        return False
        ...
        try:
            ...
            for sql in migrations:
                try:
                    ...
                    app_logger.debug(f"Migration zaten uygulanmış veya hata: {sql[:50]} → {e}")

            await db.execute("INSERT OR IGNORE INTO users (id, username, password_hash) VALUES (1, 'misafir', '')")
            await db.commit()
        finally:
            await db.close()
        app_logger.info("✅ Veritabanı başlatıldı ve migration'lar uygulandı.")
...
def migrate_data(target_user_id):
    try:
        ...
        app_logger.error(f"❌ AKTARMA HATASI: {e}", exc_info=True)
